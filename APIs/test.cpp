...
typedef void (*PTR_IMAGE_PROCESS_FUN)(struct timeval capture_time, cv::Mat &whole_img, vector<cv::Mat> &faces, cv::Mat db_embs, 
								vector<string> db_paths,  unsigned int custom_Id,vector<int> report_id);
...
void process_image(struct timeval capture_time, cv::Mat &whole_img, vector<cv::Mat> &faces,cv::Mat db_embs, 
	...
	for(int i=0;i<faces.size();++i){
		...
		string path="./result/"+to_string(report_id[i])+".bmp";
		cout<<path<<endl;
...
double get_blur_value(cv::Mat frame){  //frame is a gray image
        Mat dst;
        int kernel_size = 1;
        int ddepth = CV_16S;
        double total=0.0;
        Laplacian( frame, dst, ddepth, kernel_size);
        //int ru = int(dst.rows*0.05);
        //int rb = int(dst.rows*0.95);
        //int cl = int(dst.cols*0.1);
        //int cr = int(dst.cols*0.9);
        //Mat cropimg = dst.rowRange(ru, rb).colRange(cl, cr).clone();
        cv:Scalar tempVal = mean(dst);
        float myMAtMean = tempVal.val[0];

        for(int j=0;j<dst.rows;++j){
                 for(int k=0;k<dst.cols;++k){
                         total+=pow(abs(dst.ptr<short>(j)[k]-myMAtMean),2);
                 }
         }
         total=total/double(dst.rows*dst.cols);
        return total;
}
...
void off_video_detect(string &video_path, const DetectionConfig &detec_Config, string model_path, string db_path, 
	...
	int frame_count = 0;
	int max_age = detec_Config.max_age;
	//int min_hits = 2;
	int min_hits = detec_Config.N_frame;
	float blur_th = detec_Config.blur_th;
    ...
    for(;;) {
		...
		for(int i = 0; i < frameTrackingResult.size(); i++) { 
			//Rect_<float> face_i = frameTrackingResult[i].box;
				...
				if(!id_exists){
					...
					double blur_value = get_blur_value(t_face.aligned_face);
					t2=((double)cv::getTickCount()-t2)/cv::getTickFrequency();		
		            //cout<<"blur time:"<<t2<<endl;
					if(isface==1&&blur_value>=blur_th){
		...
		if(report_faces.size()>0){
			...
			image_Process_fun(tv, frame, report_faces, db_embs, db_paths, custom_Id, report_id);//CALLBACk Function
		}
		//double t1=(double)cv::getTickCount();
...
int main(int argc, const char* argv[])
{
	...
	try{
		...
		detec_Config.blur_th=cfg->lookupFloat(scope, "blur_th");
		detec_Config.N_frame=cfg->lookupInt(scope, "N_frame");
		detec_Config.max_age=cfg->lookupInt(scope, "max_age");
