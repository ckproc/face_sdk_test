...
void face_align(shape_predictor &sp, cv::Mat &frame, std::vector<BoundingBox> &faceinfos, std::vector<cv::Mat> &align_imgs){
	...
	for(int k=0;k<faceinfos.size();++k){
		...
		faceinfos[k].x2=std::min(faceinfos[k].x2, float(frame.cols)); //cout<<faceinfos[k].x2<<endl;
		faceinfos[k].y2=std::min(faceinfos[k].y2, float(frame.rows)); //cout<<faceinfos[k].y2<<endl;
