...
bool detect_one_face(cv::Mat raw_photo, BoundingBox &face){
	...
	float img_cw=float(c_image.cols)/2.0;
	float img_ch=float(c_image.rows)/2.0;
	//load_face_detector(model);
	//cout<<"p4"<<endl;
	...
	cand_rect.x2=std::min(cand_rect.x2, float(c_image.cols));
	cand_rect.y2=std::min(cand_rect.y2, float(c_image.rows));
	//shape_predictor sp;
	//cout<<"p7"<<endl;
	face = cand_rect;
	return true;
